...
export TARGET_DEVICE_ALT="topaz,pineapple"  # 设备代号（topaz）+ 平台码（pineapple），确保设备识别准确export FOX_VIRTUAL_AB_DEVICE=1  # 标记为虚拟A/B设备
export OF_DYNAMIC_FULL_SIZE=20401094656  # 动态分区总大小（对应 BoardConfig.mk 中 BOARD_SUPER_PARTITION_SIZE）
export FOX_VARIANT="OFRP"  # 平板设备变体（保留原有命名逻辑）
...
# （适配实际硬件：3200x2000分辨率 + 255x166mm尺寸）
# 1. 明确横屏分辨率（实际硬件3200x2000，宽>高触发横屏，宽高比16:10）
export OF_SCREEN_W=3200  # 实际屏幕横向分辨率
export OF_SCREEN_H=2000  # 实际屏幕纵向分辨率
# 2. 横屏状态栏适配（3200高分辨率优化，避免元素拥挤/贴边）
...
export OF_DEFAULT_TIMEZONE="TAIST-8;TAIDT"  # 北京时间
export OF_OPTIONS_LIST_NUM=10  # 高分辨率大屏：增加选项列表数量（适配显示空间）
export OF_DEFAULT_KEYMASTER_VERSION=4.0  # 保持原有配置
